...
def rating_to_numeric(df):
    ...
    df = df.apply(lambda col: col.map(bbg_rating_to_sp_rating))
    ...
    # Apply the mapping to each column of the dataframe
    df_numeric = df.apply(lambda col: col.map(sp_rating_to_numeric))
...
def create_rating_change_df(df, credit_event_type):
    ...
    for issuer in tqdm(df.iloc[:, :].columns, desc='Creation of {} df'.format(credit_event_type)):

        # Use NumPy arrays for faster operations
        ratings = df[issuer].values
        dates = df.index
        ...
        for i in range(max_index):
            ...
            if credit_event_type == 'upgrade':
                rating_change_detected = np.any(future_ratings < rating_tmp)
            # Assign the result
            df_credit_change.iat[i, df.columns.get_loc(issuer)] = 1 if rating_change_detected else 0
